...
ap.add_argument("-m", "--model", help="Path to minicpmv v2.0 model")
args = ap.parse_args()
...
os.system(f"cp {args.model}/tokenizer.json {args.model}/model/tokenizer.json")
with open(f"{args.model}/model/tokenizer_config.json", "r") as f:
    d = json.load(f)
    d.pop("auto_map")
    d["tokenizer_class"] = "LlamaTokenizer"
    if "add_prefix_space" in d:
        d.pop("add_prefix_space")
with open(f"{args.model}/model/tokenizer_config.json", "w") as f:
