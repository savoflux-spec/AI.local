...
- `stream_session`: a bounded ring buffer (4 MiB cap, oldest whole records drop first) plus a condvar. `append` pushes raw SSE bytes, `read_from` drains from any offset and blocks for live bytes or finalize, `finalize` wakes readers, `cancel` sets the flag the producer polls. One conv maps to at most one live session.

**UTF-8 integrity policy:** server buffers hold UTF-8 text (model output, tool output). Any operation that adds to or evicts from a buffer must not split a multi-byte UTF-8 sequence: eviction strips whole `0x0a`-terminated records only, and byte-oriented producers (e.g. `run_subprocess`) join sequences split across reads via a carry so every unit pushed is self-contained valid UTF-8. Where a guarantee cannot be made at the source, the serialization boundary handles the exception: a JSON `dump()` throw on invalid UTF-8 is caught, logged (with a hex dump of the offending bytes), and degrades to a clean stream termination instead of terminating the server process. `safe_json_to_str` (U+FFFD substitution) is reserved for diagnostics; on payload paths it masks encoding bugs instead of exposing them.
