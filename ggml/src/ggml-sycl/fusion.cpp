...
bool ggml_sycl_can_fuse(const ggml_cgraph * cgraph, int node_idx, std::initializer_list<enum ggml_op> ops,
    ...
    if (ops.size() == 2 && ops.begin()[0] == GGML_OP_SSM_CONV && ops.begin()[1] == GGML_OP_UNARY &&
        ...
        // the fused conv kernel has a silu epilogue only
        if (ggml_get_unary_op(unary) != GGML_UNARY_OP_SILU) {
            return false;
        }
        GGML_ASSERT(conv->src[0]->type == GGML_TYPE_F32);
        GGML_ASSERT(conv->src[1]->type == GGML_TYPE_F32);
        if (conv->type != GGML_TYPE_F32 || unary->type != GGML_TYPE_F32) {
            return false;
        }
        return true;
    }
